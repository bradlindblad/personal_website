...
check_url <- function(input = ".") {
  ...
  # pkg <- "foo/bar"
  dir.create("bar")
  dir.create("bar/vignettes")
  fyles <- fs::dir_ls("foo", recurse = TRUE)
  fs::file_copy(fyles, "bar/vignettes")
  
  # Flatten copy relevant files
  vig <- file.path(pkg, "vignettes")
  dir.create(vig)
  # copy_qmd_files(input, vig)
  # flatten_copy(input, vig)
  
  # Create a minimal DESCRIPTION file
  write("VignetteBuilder: knitr", file = file.path("bar", "DESCRIPTION"))
  
  # Make the copied files look like vignettes
  lapply(
    list.files("bar/vignettes"),
    function(x) {
      write(
  ...
  # fs::dir_copy("foo", "foo/bar/vignettes")
  
  urlchecker::url_check("bar")
}
